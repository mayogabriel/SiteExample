...
class RoleController extends ApiController
{
    ...
    public function index()
    {
        ...
        $includes = (Input::has('permissions') && !Input::get('permissions')) ? [] : ['permissions'];
        $roles = $roles->paginate(Input::get('limit', 50));
        
        return response()->paginator($roles, new RoleTransformer($includes));
    }
    ...
    {
        $rules = [
            'name'         => 'required|alpha_dash|min:1|max:255|unique:'.Config::get('entrust.roles_table'),
            'display_name' => 'string|max:255',
            'description'  => 'string',
            'permissions'  => 'array|integerInArray|existsInArray:'.Config::get('entrust.permissions_table').',id',
        ];
    ...
    public function update($id)
    {
        ...
        $rules = [
            'name'         => 'alpha_dash|min:1|max:255|unique:'.Config::get('entrust.roles_table').',id,' . $id,
            'display_name' => 'string|max:255',
            'description'  => 'string',
            'permissions'  => 'array|integerInArray|existsInArray:'.Config::get('entrust.permissions_table').',id',
        ];
