...
class FractalServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        response()->macro('item', function ($item, \League\Fractal\TransformerAbstract $transformer, $keyWrapperData = null, $status = 200, array $headers = []) use ($fractal, $result) {
            $resource = new \League\Fractal\Resource\Item($item, $transformer, null);
            
            if($keyWrapperData)
                $result['result'][$keyWrapperData] = $fractal->createData($resource)->toArray();
            else
        ...
        response()->macro('paginator', function ($collection, \League\Fractal\TransformerAbstract $transformer, $keyWrapperData = null, $status = 200, array $headers = []) use ($fractal, $result) {
