...
.directive('countryTimezonePickerCountries', function(countryTimezonePickerService) {
    ...
    return {
        template: '<select material-select name="country" ng-model="selectedValue" ng-options="country.code as country.name for country in countries" required=""></select>',
        restrict: 'E',
        replace: true,
        scope: {
...
.directive('countryTimezonePickerTimezone', function(countryTimezonePickerService, $filter) {
    ...
    return {
        template: '<select material-select ng-model="selectedValue" ng-options="timezone.name as timezone.displayName for timezone in timezones" required="" id="timezone" name="timezone"></select>',
        restrict: 'E',
        replace: true,
        scope: {
        ...
        link: function (scope, elem, attrs) {
            scope.$watch('selectedCountry', function(country) {
                ...
                if (scope.timezones.length > 0 
                    && $filter('filter')(scope.timezones, { name: scope.selectedValue }).length === 0 ){
            ...
            if (scope.selectedCountry) {
                ...
                if (_.size(scope.timezones) > 0 
                    && $filter('filter')(scope.selectedValue, { name: scope.selectedValue }).length === 0) {
                    
                    if(angular.isDefined(scope.timeNgModulezones)) {
                        scope.selectedValue = scope.timeNgModulezones[0].name;                    
                    }
                }                
            }
        }
